...
generateRouter.post('/', authenticate, authorize('ADMIN', 'GENERATOR'), generateRateLimiter, async (req, res, next) => {
    try {
        ...
        if (todayCount + config.totalQuestions > (org?.maxQuestionsPerDay ?? 200)) {
            throw new AppError(`Daily generation quota exceeded. Used ${todayCount} of ${org?.maxQuestionsPerDay}.`, 429);
        }
        // Estimate cost
