...
function buildDSASystemPrompt(config) {
...
function buildOOPSSystemPrompt(config) {
...
export function getLLMClient(provider, _organizationId) {
    ...
    switch (provider) {
        case 'anthropic': {
            ...
            if (!key)
                throw new AppError('ANTHROPIC_API_KEY not configured', 400);
            return new AnthropicLLMClient(key);
        }
        case 'openai': {
            const key = process.env.OPENAI_API_KEY;
            if (!key)
                throw new AppError('OPENAI_API_KEY not configured', 400);
