...
import { LayoutDashboard, Wand2, FileQuestion, ClipboardCheck, BookOpen, BarChart3, Settings, LogOut, ShieldCheck } from 'lucide-react';
const navItems = [
    ...
    return (<div className="app-layout">
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          {navItems.map(item => (<NavLink key={item.to} to={item.to} end={item.exact} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
              <item.icon size={16}/>
              {item.label}
            </NavLink>))}
          ...
          {user?.role === 'ADMIN' && (<>
              ...
              {adminItems.map(item => (<NavLink key={item.to} to={item.to} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                  <item.icon size={16}/>
                  {item.label}
                </NavLink>))}
            </>)}
