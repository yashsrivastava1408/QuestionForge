...
export default function QuestionsPage() {
    ...
    return (<>
      ...
      <div className="page-body">
        ...
        ].map(f => (<select key={f.key} className="form-select" style={{ width: 160 }} value={filters[f.key]} onChange={e => setFilters(prev => ({ ...prev, [f.key]: e.target.value, page: 1 }))}>
        ...
        {isLoading ? (<div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div className="table-wrapper">
              <table>
                ...
                <tbody>
                  {questions.length === 0 ? (<tr><td colSpan={7} style={{ textAlign: 'center', padding: 40, color: 'var(--color-text-muted)' }}>No questions found.</td></tr>) : questions.map((q) => (<tr key={q.id}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 20 }}>
              ...
              <button className="btn btn-secondary btn-sm" disabled={filters.page * 20 >= total} onClick={() => setFilters(f => ({ ...f, page: f.page + 1 }))}>Next →</button>
            </div>
          </>)}
      </div>
    </>);
