...
export default function ReviewPage() {
    ...
    const reviewMutation = useMutation({
        mutationFn: ({ id, decision, note }) => axios.post(`/api/questions/${id}/review`, { decision, note }),
    ...
    return (<>
      ...
      <div className="page-body">
        {questions.length === 0 ? (<div className="empty-state">
            <CheckCircle size={48}/>
            ...
            {questions.map((q) => (<div key={q.id} className="question-card animate-fade-in">
                <div className="question-card-header">
                  <div className="question-card-title">{q.title}</div>
                  <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)' }} onClick={() => setExpanded(expanded === q.id ? null : q.id)}>
                ...
                <div className="question-card-meta">
                  ...
                  {q.validationResult?.crossCheckPassed && (<span style={{ fontSize: 12, color: 'var(--color-success)', display: 'flex', alignItems: 'center', gap: 4 }}>
                      <CheckCircle size={12}/> Cross-check passed
                    </span>)}
                </div>

                {expanded === q.id && (<div style={{ marginTop: 16, animation: 'fadeIn 0.2s ease' }}>
                    ...
                    {q.validationResult && (<div style={{ fontSize: 13, color: 'var(--color-text-secondary)', marginBottom: 12 }}>
                        <strong>Validation:</strong> {q.validationResult.details}
                      </div>)}
                  </div>)}

                <div className="question-card-actions">
                  <button className="btn btn-success btn-sm" onClick={() => reviewMutation.mutate({ id: q.id, decision: 'APPROVED' })} disabled={reviewMutation.isPending}>
                    <CheckCircle size={14}/> Approve
                  </button>
                  <button className="btn btn-danger btn-sm" onClick={() => reviewMutation.mutate({ id: q.id, decision: 'REJECTED' })} disabled={reviewMutation.isPending}>
